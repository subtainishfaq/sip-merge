...
public class  FavoriteContact extends JDialog
{
	 ...
	 */
	public FavoriteContact(Frame parent, boolean modal)
	{
		super(parent, modal);
		initComponents();
		pack();
	...
	private void initComponents()
	{
		...
		// have a description
		getAccessibleContext().setAccessibleDescription("Favorite");
		...
		submitButton.addActionListener(new ActionListener()
		{
			...
			{	
				String faved = userNameTextField.getText();
				String user = Global.currentUser;
				JFrame frame = new JFrame();
				if (faved.equals("") || (faved==null))  {
					...
					if (favoriteman.checkIfUserExists(faved)){
						if (favoriteman.addFavoriteToDB(user, faved)){
							JOptionPane.showMessageDialog(frame, "User "+faved+" has been added to Favorites List","Favorite Status", JOptionPane.INFORMATION_MESSAGE);
							dialogDone(event);
						}
						else{
